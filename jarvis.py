...
def sendEmail(to, content):
    server = smtplib.SMTP('aryanak919168@gmail.com', 587)
    server.ehlo()
    server.starttls()
    server.login("aryanak9168@gmail.com", "aryan1234")
    server.sendmail("aryanak9168@gmail.com", to, content)
    server.close()
...
def screenshot():
    ...
    img.save(
        "C:\\Users\\AI-AI-using-python3-\\screenshots\\ss.png"
    )
...
# weather condition
def weather():
    api_key = "4cbc3a6b7db914cadda17514b61555c0"  # generate your own api key from open weather
    # base_url = f'http://api.openweathermap.org/data/2.5/weather?'
    # speak("tell me which city")
    # city_name = takeCommand()
    # complete_url = base_url + "appid=" + api_key + "&q=" + city_name + "&units=metric"
    complete_url = f'http://api.openweathermap.org/data/2.5/weather?q=Patna&appid={api_key}&units=metric'
    ...
    weather_paragraph = f"In Patna, the weather is currently {weather_description}. " \
                        f"The temperature is {temperature}°C with a humidity of {humidity}%. " \
                        f"The wind speed is {wind_speed} meters per second."
    print(weather_paragraph)
    speak(weather_paragraph)



def personal():
    speak(
        "I am AI, version 1.0, I am an AI assistant, I am developed by Techie bot for smart eyeglesses under smart Bihar hackathon 2023"
    )
    ...
    while True:
        ...
        # cpu and battery usage
            ...
            cpu()
                ...
                "read book" in query or "red book" in query or "read text" in query or "read text" in query or "recognize text" in query):
        ...
        elif ('i am done' in query or 'bye bye AI' in query
              or 'go offline AI' in query or 'bye' in query
              or 'nothing' in query):
            wishme_end()
